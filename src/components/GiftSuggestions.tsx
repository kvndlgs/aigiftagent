...
export const MockGiftSuggestions: React.FC = () => {
  const { mockSuggestions, isMockLoading } = useMockGiftStore();
  if(isMockLoading) {
    return (
      <div>
    ...
    if (!mockSuggestions.length) {
      return null;
    }
  
    return (
      <div className="w-full max-w-6xl space-y-8">
        {mockSuggestions.map((mockSuggestion, index) => (
          <div key={index} className="bg-gray-50 rounded-xl shadow-lg p-8">
            <div className="flex items-center mb-6">
              <Package className="w-7 h-7 text-indigo-600" />
              <h3 className="text-2xl font-semibold ml-2">
                {mockSuggestion.occasion} Gift Ideas
              </h3>
            </div>
  
            <div className="grid gap-6 md:grid-cols-3">
              {mockSuggestion.ideas.map((idea, ideaIndex) => (
